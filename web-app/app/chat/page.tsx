...
export default function Chat() {
  ...
  const [socket, setSocket] = useState<any>(
    io('wss://sbbj-chats-service-27br4jjo7a-uc.a.run.app', {
  ...
  useEffect(() => {
    const newSocket = io('wss://sbbj-chats-service-27br4jjo7a-uc.a.run.app', {
      transports: ['websocket'],
    })
    setSocket(newSocket)
  ...
  const loadMoreMessages = async () => {
    ...
    const moreMessages = await chatService
      .getMsg(room, count + 1)
      .then((msg) => {
    ...
    setChatMessages((prevMessages) => {
      ...
      setTimeout(() => {
        chatContainerRef.current.scrollTop =
          chatContainerRef.current.scrollHeight * newMessagesPercentage
      }, 0)
      ...
      <div className={styles.container}>
        ...
        <div className={styles.container} ref={chatContainerRef}>
          {chatMessages.map((data, index) => (
            ...
            >
              {data.name != 'Admin' && (
                <div className={styles.postHeaderName}>{data.name}</div>
              )}
